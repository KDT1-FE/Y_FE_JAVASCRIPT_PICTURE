...
db.collection('profile')
  ...
  .then((res) => {
    res.forEach((item) => {
      ...
      const profile = storageRef.child(
        'image/' + item.data().employeeId + '.jpg'
      );
...
async function deleteFirestore(collection, doc) {
...
document.querySelector('.btn__del').addEventListener('click', async () => {
  let checkIds = await getCheckEmployeeId();
  const userConfirmed = confirm(
    `직원 ${checkIds.length}명을 삭제하시겠습니까?`
  );
  if (!userConfirmed) {
    return;
  }
  try
  {
    ...
    for (const checkId of checkIds) {
      await deleteEmployeeData(checkId);
    } 
...
function deleteEmployeeData(checkId) {
    ...
    if (e.target.value.length >= 2) {
      tableRow.forEach((item) => {
        ...
        if (employeeName.innerHTML.includes(e.target.value)) {
          employee.classList.contains('hide') &&
