...
async function updateImgInFirestore(docId, imagePath) {
    try {
        const imageRef = (0, _storage.ref)(storage, imagePath);
        const downloadURL = await (0, _storage.getDownloadURL)(imageRef);
        const userDocRef = (0, _firestore.doc)(db, "database", docId);
...
async function getDocumentIdById(id) {
    const querySnapshot = await (0, _firestore.getDocs)((0, _firestore.query)((0, _firestore.collection)(db, "database"), (0, _firestore.where)("id", "==", id)));
    if (!querySnapshot.empty) return querySnapshot.docs[0].id; // 첫 번째 문서의 ID를 반환합니다.
    return null; // 해당 id와 일치하는 문서가 없을 경우 null을 반환합니다.
}
// 상세 버튼 클릭 시 상세 페이지로 이동하는 함수
function redirectToProfilePage(id) {
    getDocumentIdById(id).then((documentId)=>{
        if (documentId) window.location.href = `profile.html?documentId=${documentId}`;
        else console.log("해당 id와 일치하는 문서가 없습니다.");
    }).catch((error)=>{
